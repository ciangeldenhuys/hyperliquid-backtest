...
class VolumeCollecterTest:

    def __init__(self, coin: str, bt: Backtest):
        self.coin = coin
    ...
    async def run(self):
        await asyncio.gather(self.stream_HL_trades(), self.flush(), self.bt.run())
    
if __name__ == "__main__":
    bt = Backtest(datetime(2025, 1, 1), datetime(2025, 1, 2), 'XRPUSDT')
    vc = VolumeCollecterTest("@151", bt)
    asyncio.run(vc.run())
