...
import asyncio
from volume_collect_test import VolumeCollecterTest
...
class VolumeSignal:
    ...
    def __init__(self, symbol: str, threshold: float, min_points: int, poll_interval: float, usd_notional: float, Z_SCORE_MAX: int, start: datetime, end: datetime):
        """
        Initialize a VolumeSignal instance.
        """
        self.collector = VolumeCollecterTest(symbol)
...
if __name__ == "__main__":
    symbol = "@151"
    vs = VolumeSignal(symbol=symbol, threshold=3.0, min_points=10, poll_interval=30, usd_notional=1000, Z_SCORE_MAX=5)
